...
#ifdef _WIN32
static std::vector<HANDLE> handleStore;

int putHandle( HANDLE h )
{
...
inline int int_cmper(const void* p1, const void* p2)
{
	int* d1 = (int*)p1;
	int* d2 = (int*)p2;
	return *d1 - *d2;
...
int AlgorithmAPI::ShadowsFilter(UserData* scan_in, const ShadowsFilterParam& param)
{
	...
	for (size_t i = 0; i < scan_in->framedata.data.size() - param.window - 1; i++)
	{
...
int SystemAPI::GetComList(std::vector<UARTARG>& list)
{
	...
	{
		UARTARG arg;
		for (size_t j = 0; j < port_list.size(); j++)
		{
...
int32_t GetDevInfoByUART(const char* port_str, uint32_t speed)
{
	...
	if (rf > 10)
	{
		for (size_t idx = 0; idx < rf - 10; idx++)
		{
